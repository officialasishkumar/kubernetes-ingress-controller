...
func TestConsumerGroup(t *testing.T) {
	...
	for _, consumer := range consumers {
		require.Eventually(t, func() bool {
			...
			resp, err := helpers.DefaultHTTPClient(helpers.WithResolveHostTo(proxyHTTPSURL.String()), helpers.WithInsecureSkipVerify()).Do(req)
			if err != nil {
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		clearResp, err := helpers.DefaultHTTPClient(helpers.WithResolveHostTo(proxyHTTPSURL.String()), helpers.WithInsecureSkipVerify()).Do(clearReq)
		if !assert.NoError(c, err) {
			return
		}
		...
		emptyResp, err := helpers.DefaultHTTPClient(helpers.WithResolveHostTo(proxyHTTPSURL.String()), helpers.WithInsecureSkipVerify()).Do(empty)
		if !assert.NoError(c, err) {
			return
		}
