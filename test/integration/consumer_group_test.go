...
func TestConsumerGroup(t *testing.T) {
	...
	for _, consumer := range consumers {
		require.Eventually(t, func() bool {
			req := helpers.MustHTTPRequest(t, http.MethodGet, proxyHTTPSURL.Host, path, map[string]string{
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		req := helpers.MustHTTPRequest(t, http.MethodGet, proxyHTTPSURL.Host, multiPath, map[string]string{
			"apikey": four.Name,
		})
		resp, err := helpers.DefaultHTTPClient(helpers.WithResolveHostTo(proxyHTTPSURL.Host), helpers.WithInsecureSkipVerify()).Do(req)
		...
		clearReq := helpers.MustHTTPRequest(t, http.MethodGet, proxyHTTPSURL.Host, path, map[string]string{
			"apikey": four.Name,
		})
		clearResp, err := helpers.DefaultHTTPClient(helpers.WithResolveHostTo(proxyHTTPSURL.Host), helpers.WithInsecureSkipVerify()).Do(clearReq)
		if !assert.NoError(c, err) {
			return
		}
		...
		empty := helpers.MustHTTPRequest(t, http.MethodGet, proxyHTTPSURL.Host, multiPath, map[string]string{
			"apikey": "test-consumer-3",
		})
		emptyResp, err := helpers.DefaultHTTPClient(helpers.WithResolveHostTo(proxyHTTPSURL.Host), helpers.WithInsecureSkipVerify()).Do(empty)
		if !assert.NoError(c, err) {
			return
		}
