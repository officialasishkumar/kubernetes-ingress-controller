...
func TestPluginConfigPatch(t *testing.T) {
	...
	t.Log("waiting for routes from Ingress to be operational")
	assert.Eventually(t, func() bool {
		resp, err := helpers.DefaultHTTPClient(helpers.WithInsecureSkipVerify()).Get(fmt.Sprintf("%s/test_plugin_config_patch", proxyHTTPSURL))
		if err != nil {
			t.Logf("WARNING: error while waiting for %s: %v", proxyHTTPSURL, err)
			return false
		}
	...
	t.Log("deleting Ingress and waiting for routes to be torn down")
	require.NoError(t, clusters.DeleteIngress(ctx, env.Cluster(), ns.Name, ingress))
	helpers.EventuallyGETPath(t, nil, proxyHTTPSURL.String(), "/test_plugin_config_patch", &helpers.HTTPSOptions{InsecureSkipVerify: true}, http.StatusNotFound, "no Route matched with those values", nil, ingressWait, waitTick)
}
...
func TestPluginOrdering(t *testing.T) {
	...
	helpers.EventuallyExpectHTTP404WithNoRoute(
		t, proxyHTTPSURL, proxyHTTPSURL.Host, "/test_plugin_ordering", nil, ingressWait, waitTick, nil,
	)
}
...
func TestPluginNullInConfig(t *testing.T) {
	...
	t.Log("waiting for routes from Ingress to be operational")
	assert.Eventually(t, func() bool {
		resp, err := helpers.DefaultHTTPClient(helpers.WithInsecureSkipVerify()).Get(fmt.Sprintf("%s/test_plugin_null_in_config", proxyHTTPSURL))
		if err != nil {
			t.Logf("WARNING: error while waiting for %s: %v", proxyHTTPSURL, err)
			return false
		}
